...
let productArr=[];
let total;
let numberOfPages=Math.ceil(total/itemPerpage);
let currentPage=1;
...
document.addEventListener("DOMContentLoaded", fetching_product(currentPage));
...
function addProductToTable(product, index) {
  ...
  // } else {
    ...
    addToCart.addEventListener("click", function () {
      addingToCart(product.title,product.price,buying_qt.value,product.thumbnail);
    });
  // }
...
const product_box = document.querySelector(".product_box");
//ajax
function fetching_product(currentPage) {
    allProduct.innerHTML=``;
  const fetch_btn = document.querySelector("#fetch-btn");
  const all_product = document.querySelector(".allProduct");
  fetch("http://localhost:3000/api/products")
    ...
    .then((res) => {
      ...
      displayRecords(currentPage,res.products);
...
function onNext()
{
 ...
 currentPage++;
fetching_product(currentPage);
 renderPagination();
   if(currentPage===Math.ceil(total/itemPerpage))
  {
 ...
 console.log(Math.ceil(total/itemPerpage))
}
...
// displayRecords(page);
function displayRecords(currentPage,products)
{
    const startIndex=(currentPage-1)*itemPerpage;
    const endIndex=currentPage*itemPerpage;
    visibleItem=products.slice(startIndex,endIndex);
    console.log(visibleItem);
    visibleItem.forEach(product => {
          addProductToTable(product);      
    });
    addProductToTable(visibleItem);
